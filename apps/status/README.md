...
- ティッカーは [shared/schedule.ts](../../shared/schedule.ts) のタイムテーブルを見て `<まもなく|開催中|次は> <時刻> <企画> @ <会場>` を出す。開始 10 分前から終了までが対象で、開催中は現在の枠と次の枠を 10 秒ごとに入れ替える。予定が無い時間帯は管理画面の固定案内に戻り、速報はどちらも上書きする
- 開催日以外でフッターの見た目を確認するときは `?at=2026-09-26T10:22`（端末のタイムゾーンで解釈）を付ける
