...
- 「ステータス」は固定の grid で、ステータス送信（団体のセレクト付き）と更新履歴が横並びで、その下に最終更新のグラフがあります。「設定」は見出しで区切った「サイネージ」（フッター・閲覧 URL・動画・音声）と「ステータス」（受付時間・テスト受付）が縦に並び、区切り線はウィンドウ同士の間にだけ引きます。テスト受付は受付時間の左下から確認モーダルで開始・終了します。
...
- 「最終更新」で全団体の最後の送信時刻を見られます。`GET /api/statuses` で 60 秒ごとに全団体をまとめて取り、「ステータス」タブが見えていない間は止めます。
...
- `signage_config.video_stop_at` を過ぎると動画を消して `放映終了` を出します。null なら止めません。開始時刻より後でなければ保存できません。
...
| `GET /api/statuses` | Admin Bearer             | 全団体の現在値                             |
