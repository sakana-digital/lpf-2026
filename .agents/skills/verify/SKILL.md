...
- Events grid: `?by=category` regroups the rows by category (`.segmented-switch` above the grid); the default lists grades, clubs and committees. Click a `.events-grid .cell .org-trigger` → row/column expand + `?org=<id>` in URL; deep link `/explore/events?org=c1-3` expands on load.
- Timetable: click a `.slot.linked .org-trigger` to expand; the detail sits outside the trigger.
